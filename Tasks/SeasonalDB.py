...
if AssetName == "ScrewCompressor" :
    ...
    bigdata.to_csv("FinalPredictionScrew.csv")
    ...
    bigdata.to_csv("FinalPredictionCentrifugal.csv")
    for i, row in bigdata.iterrows():
    ...
    query='''SELECT * FROM ReciprocatingProcessedTables WHERE RPId={}'''
    series = pd.read_sql(query.format(batchId),conn, parse_dates=['Date'])
    startdate=min(series["Date"])
    series.set_index('Date')
    # Step 3 :- 
    ...
    bigdata.to_csv("FinalPredictionReciprocating.csv")
    for i, row in bigdata.iterrows():
      if pd.isnull(row['TDValve']):
        row['TDValve']=0
      SQLCommand = "INSERT INTO ReciprocatingPredictedTables(RPId,Date,TDValve) VALUES('" + str(batchId) + "','" + str(i) + "','" + str(row['TDValve']) + "')"
      cursor.execute(SQLCommand)
    conn.commit()